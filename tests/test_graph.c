...
main(int argc, char **argv) {
    ...
    * Vertex F (i:5): [ B (i:1), 2 ] , [ C (i:2), 6 ] , [ D (i:3), 8 ] , [ G (i:6), 9 ]
   ...
   printf("\n\n !! Changing a to visited\n");
   /* change vertex */
   graph_vertex_array[0]->visited = TRUE;

   /* test re-direction */
   if(gstruct->vertices[1]->vneighbors[0].vpointer->visited == TRUE)
      {printf("\n\t -- Vertex %s says neighbor %s is visited: YES", 
            gstruct->vertices[1]->vname, gstruct->vertices[1]->vneighbors[0].vpointer->vname);}
   else
      {printf("\n\t -- Vertex %s says neighbor %s is visited: NO", 
            gstruct->vertices[1]->vname, gstruct->vertices[1]->vneighbors[0].vpointer->vname);}

   printf("\n\n !! Changing A to not visited\n");
   gstruct->vertices[0]->visited = FALSE;
   if(gstruct->vertices[2]->vneighbors[0].vpointer->visited == TRUE) 
      {printf("\n\t -- Vertex %s says neighbor %s is visited: YES", 
            gstruct->vertices[2]->vname, gstruct->vertices[2]->vneighbors[0].vpointer->vname);}
   else
      {printf("\n\t -- Vertex %s says neighbor %s is visited: NO", 
            gstruct->vertices[2]->vname, gstruct->vertices[2]->vneighbors[0].vpointer->vname);}
